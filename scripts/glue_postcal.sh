...
OUTDIR=./exps/glue_postcal_$MM/$TASK_NAME/LR${LR}BA${BA}MAXSTEP${MAXSTEP}WARMSTEP${WARMSTEP}PCALLAMBDA${LAMBDA}TYPE${TYPE}
echo OUTDIR is $OUTDIR 
sleep 3

if [ $1 == 'train' ]; then
	echo doing train with postcal
	echo LAMBDA: $LAMBDA TYPE: $TYPE
	sleep 1
	python nce_glue/run_glue.py --model_name_or_path roberta-base --task_name $TASK_NAME --overwrite_cache --do_train --do_eval_calibration --data_dir $GLUE_DIR/$TASK_NAME --max_seq_length 128 --per_device_train_batch_size $BA --learning_rate $LR --output_dir $OUTDIR --overwrite_output_dir --logging_steps 50 --save_steps -1 --warmup_steps $WARMSTEP --max_steps $MAXSTEP --weight_decay 0.1 --evaluate_during_training --eval_steps 500 --pcalloss_lambda $LAMBDA --pcal_train --pcalloss_type $TYPE
